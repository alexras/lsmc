...
def test_rle_compress():
    ...
    assert_equal(compressed, reference)
...
def test_default_instr_compress():
    ...
    assert_equal(decompressed_reference, data + [0])

    data = bytearray(data)

    compressed = filepack.compress(data, test=True)

    assert_equal(bytearray(compressed + [0]), reference)

    decompressed = filepack.decompress(compressed)

    assert_equal(data, bytearray(decompressed))
...
def test_instrument_sizes():
    ...
    assert_equal(raw_data, bytearray(filepack.DEFAULT_INSTRUMENT))
...
def test_default_wave_compress():
    ...
    decompressed_reference = filepack.decompress(reference)
    assert_equal(decompressed_reference, data)
...
def test_bad_rle_split():
    ...
    assert_raises(AssertionError, filepack.split, data, bl.BLOCK_SIZE,
                  factory)
...
def test_special_byte_on_block_boundary():
    ...
    assert_equal(factory.blocks[0].data, block_0_expected)
    assert_equal(factory.blocks[1].data, block_1_expected)
    assert_equal(factory.blocks[2].data, block_2_expected)
...
def test_merge_with_rle_byte():
    ...
    assert_equal(data, [filepack.RLE_BYTE, filepack.RLE_BYTE, 2, 1, 3, 4, 3, 6])
...
def test_merge_with_full_rle():
    ...
    assert_equal(data, [filepack.RLE_BYTE, 42, 17, 1, 1, 4, 4, 4, 42])
...
def test_decompress_bogus_special_byte_asserts():
    ...
    assert_raises(AssertionError, filepack.decompress, data)
...
def test_weird_rle_compress():
    ...
    for i in xrange(max(len(data), len(recompressed))):
        assert_equal(data[i], recompressed[i])
...
def test_sample_song():
    ...
    assert_equal(recompressed, compressed)
